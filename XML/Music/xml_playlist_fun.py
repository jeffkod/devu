...
def findall_example_playlist(root):
	songs = root.findall('Artist')
	for item in songs:
		print()

def  dfs_iter_playlist(tree):     # note that this takes the entire tree and not the root. no need for nested for loops
	for elem in tree.iter():
		print(elem.tag,elem.attrib,elem.text)

#traverse_the_tree(root)
#traverse_the_tree_and_write_to_text(root)
#findall_example_playlist(root)
#dfs_iter_playlist(tree)
